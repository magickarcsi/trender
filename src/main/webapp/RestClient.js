...
// Model for Names entity
    models.Names = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/trender/webresources/com.opst.names/",
        idAttribute: 'id',
        defaults: {
            isActive: "",
            store: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/trender/webresources/com.opst.names/';
            }
    ...
    // Collection class for Names entities
    models.NamesCollection = Backbone.Collection.extend({
        model: models.Names,
        url: "http://localhost:8080/trender/webresources/com.opst.names/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.NamesCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-names-list-itemis template identifier for item
                        templateName: '#tpl-names-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.names = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.names,
                    // tpl-names-details is template identifier for chosen model element
                    templateName: '#tpl-names-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Names();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-names-details is a template identifier for chosen model element
                templateName: '#tpl-names-details',
